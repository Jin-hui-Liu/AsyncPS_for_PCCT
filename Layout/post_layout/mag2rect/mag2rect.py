...
# Added ndiff, pdiff, poly, locali, viali. Modified ndiffc, pdiffc, polycont
rect_map =  {
  'locali'      :   ['m1'],
  'metal1'      :   ['m2'],
  'metal2'      :   ['m3'],
  ...
  'pdiff'       :   ['pdiffusion'],
  'poly'        :   ['polysilicon'],
  'ndiffc'      :   ['ndc','ndiffusion', 'm1'],
  'ndc'         :   ['ndc','ndiffusion', 'm1'],
  'pdiffc'      :   ['pdc','pdiffusion', 'm1' ],
  'pdc'         :   ['pdc','pdiffusion', 'm1'],
  'polycont'    :   ['pc','polysilicon', 'm1'],
...
#LJH: Add 'r' at the front so that python will treat the latter string as "raw string", resulting in no warnings towards the '\s'
parse_layer = r'<<\s+(?P<layer>\w+)\s+>>'
parse_coord = r'rect\s+(?P<xl>[0-9-]+)\s+(?P<yl>[0-9-]+)\s+(?P<xh>[0-9-]+)\s+(?P<yh>[0-9-]+)'
parse_label = r'[rf]label\s*(?P<layer>\w+)\s*(?P<xl>[0-9-]+)\s+(?P<yl>[0-9-]+)\s+(?P<xh>[0-9-]+)\s+(?P<yh>[0-9-]+)\s+\d+\s+(?:FreeSerif\s+120\s+0\s+0\s+0\s+)?(?P<label>[a-zA-Z0-9_()#]+)'

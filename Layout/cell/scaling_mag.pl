...
my $scale_factor = $ARGV[$#ARGV] || 1; # Default to 1 if second argument is not provided or invalid
# $scale_factor = int($scale_factor); # Convert to integer

foreach $i (@ARGV) {
	if ($i ne $scale_factor) {
		&process_rect ($i);
	}
}
&std_defs ();
...
sub process_rect {
	...
	$urx *= $scale_factor;
	$ury *= $scale_factor;
#	unless (is_integer($llx) && is_integer($lly) && is_integer($urx) && is_integer($ury)) {
#		warn "Error: Non-integer coordinates after scaling\n";
#	}
